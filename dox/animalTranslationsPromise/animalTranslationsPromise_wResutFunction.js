...
function gatherResult(result, resultObject) {
    resultObject.returnObject.animal = myAnimal;
    resultObject.returnObject.sound = result;
    resultObject.returnObject.sentemce = 'The ' + myAnimal + ' says,"' + result + '."';
    let vowels = ['A', 'E', 'I', 'O', 'U'];
    let soundFirstLetter = result.substr(0, 1);
    console.log(soundFirstLetter)
    let article = vowels.includes(soundFirstLetter) ? 'an' : 'a';
    resultObject.returnObject.article = article;
    resultObject.returnObject.oldMacDonald = [
